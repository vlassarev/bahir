...
import org.apache.bahir.utils.{Logging, TlsUtil}
...
class MQTTStreamSourceProvider extends StreamSourceProvider with DataSourceRegister with Logging {
  ...
  override def createSource(sqlContext: SQLContext, metadataPath: String,
    ...
    val cacrtfile: Option[String] = parameters.get("cacrtfile")
    val crtfile: Option[String] = parameters.get("crtfile")
    val keyfile: Option[String] = parameters.get("keyfile")
    (cacrtfile, crtfile, keyfile) match {
      case (Some(cc: String), Some(cf: String), Some(kf: String)) =>
        mqttConnectOptions.setSocketFactory(TlsUtil.getSocketFactory(cc, cf, kf, ""))
      case _ =>
    }
